...
class GenomicData(object):
    ...
    error_rate: the error_rate, extracted from model_params
    """
    def __init__(self):
...
class GenomicDataVDJ(GenomicData):
    ...
    def load_igor_genomic_data(self, params_file_name, V_anchor_pos_file, J_anchor_pos_file):
        ...
        self.read_error_rate(params_file_name)
        self.generate_cutV_genomic_CDR3_segs()
        self.generate_cutD_genomic_CDR3_segs()
        self.generate_cutJ_genomic_CDR3_segs()

    def read_error_rate(self, params_file_name):
        with open(params_file_name) as f:
            for l in f:
                if l.startswith("#SingleErrorRate"):
                    break
            self.error_rate = float(f.readline())
